...
- [[#motivation][Motivation]]
  ...
  - [[#what-now][What now?]]
- [[#usage][Usage]]
  - [[#quickstart-basic-usage][Quickstart (Basic Usage)]]
  - [[#section-definition][Section Definition]]
  - [[#dependencies--inheritance][Dependencies & Inheritance]]
  - [[#allowing--disallowing-sections][Allowing & Disallowing Sections]]
  - [[#mutually-exclusive-sections][Mutually Exclusive Sections]]
  - [[#overriding-sections][Overriding Sections]]
...
#+begin_src emacs-lisp
  (guard-allow org)
#+end_src
Allowing ~org~ will enable it and also enable all of its children transitively (look at the usage below for all of the rules). Everything is enabled by default, so allowing ~org~ won't be needed if we haven't disabled any of its parents.
Do I want to disallow everything org-related but only enable org-roam (for some reason)?
#+begin_src emacs-lisp
  (guard-disallow org
    (guard-allow org-roam))
#+end_src
I chose to allow nesting these statements to visually highlight the fact that we are trying to say "from org allow org-roam". The same could be achieved by:
#+begin_src emacs-lisp
  (guard-disallow org)
  (guard-allow org-roam)
#+end_src
...
#+end_src
and ~org-latex~ is replaced completely with the new config. If I want to keep it, but also prepend something to that section:
#+begin_src emacs-lisp
...
** What now?
We have some nice infrastructure we can build some stuff on top of (and ideas are always welcome). So far these are:
...
- More introspection: Calling ~guard-look~ and selecting a section jumps to a buffer explaining what is up with that section, including its docstring, its status (enabled/disabled), parents and children.
...
Any allowing, disallowing or overriding has to happen *before* the main configuration. For ease of use, put all of the allowing, disallowing and overriding in the init-tweak file.
As an example, this is the core of my init.el (skipping loading ~use-package~ and enabling MELPA).

#+BEGIN_SRC emacs-lisp
  (use-package guard
    :ensure t)

  ;; Initialize the graph, has to be called before `guard-config`.
  (guard-initialize)
  ;; Load the tweak file where enabling, disabling, overriding happens.
  (guard-config)
  ;; The main configuration where the sections live.
  ;; Mine is in a large org file so I load it.
  (org-babel-load-file
   (expand-file-name "resources/config/myinit.org" user-emacs-directory)) ;
#+END_SRC
